...
class PoseDetectionManager:
    ...
    def get_latest_result(self, frame_id=None):
        """获取最新的检测结果"""
        if frame_id and frame_id in self.latest_results:
            return self.latest_results[frame_id]
        
        # 返回最近的结果
        if self.latest_results:
            latest_id = max(self.latest_results.keys())
            return self.latest_results[latest_id]
